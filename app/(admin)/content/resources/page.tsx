...
import { Trash2, Edit2, Plus, File } from "lucide-react";
...
export default function ResourcesPage() {
  ...
  const [formData, setFormData] = useState({ title: "", coverImageUrl: "", fileUrl: "", fileType: "", categoryTag: "" });
  ...
  const handleSave = async (e: any) => {
    ...
    if (editingId) {
      ...
      await addDoc(collection(db, "resources"), { ...formData, createdAt: Date.now() });
    }
  ...
  return (
    <div className="flex flex-col h-full font-inter">
      <div className="flex items-center justify-between mb-8">
        <h2 className="text-3xl font-montserrat font-bold text-[#0F172A] tracking-tight">Resources</h2>
        <button onClick={() => { setFormData({title:"", coverImageUrl:"", fileUrl:"", fileType:"", categoryTag:""}); setEditingId(null); setIsModalOpen(true); }} className="bg-[#F59E0B] text-white px-6 h-14 rounded-[18px] font-semibold flex items-center gap-2 hover:scale-[1.03] shadow-[0_10px_40px_rgba(245,158,11,0.3)] transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 overflow-y-auto pb-8">
        {items.map(item => (
          <div key={item.id} className="bg-white/55 backdrop-blur-[24px] border border-white/35 rounded-[28px] overflow-hidden shadow-[0_10px_40px_rgba(15,23,42,0.08)] hover:-translate-y-2 hover:shadow-[0_25px_60px_rgba(15,23,42,0.16)] transition-all p-6 flex items-center gap-4">
            <div className="h-16 w-16 rounded-[16px] bg-slate-100 flex-shrink-0 flex items-center justify-center overflow-hidden">
              {item.coverImageUrl ? <img src={item.coverImageUrl} className="w-full h-full object-cover" /> : <File className="text-slate-400 w-8 h-8"/>}
            </div>
            <div className="flex-1 min-w-0">
              <h3 className="font-bold text-[#0F172A] truncate">{item.title}</h3>
              <div className="flex items-center gap-2 mt-1">
                <span className="text-[10px] font-bold uppercase tracking-widest bg-blue-100 text-blue-700 px-2 py-1 rounded-md">{item.categoryTag}</span>
                <span className="text-[10px] font-bold uppercase tracking-widest bg-slate-100 text-slate-600 px-2 py-1 rounded-md">{item.fileType}</span>
              </div>
              <div className="flex items-center gap-3 mt-3">
                <button onClick={() => { setFormData(item); setEditingId(item.id); setIsModalOpen(true); }} className="text-blue-500 hover:text-blue-700 text-[10px] font-bold uppercase"><Edit2 className="w-3 h-3 inline mr-1"/> Edit</button>
                <button onClick={() => { if(confirm("Delete?")) { deleteDoc(doc(db, "resources", item.id)); fetchItems(); } }} className="text-red-500 hover:text-red-700 text-[10px] font-bold uppercase"><Trash2 className="w-3 h-3 inline mr-1"/> Delete</button>
              </div>
            </div>
          </div>
        ))}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#020617]/40 backdrop-blur-sm p-4">
          <form onSubmit={handleSave} className="bg-white/95 backdrop-blur-3xl border border-white/40 p-8 rounded-[32px] w-full max-w-lg shadow-[0_25px_60px_rgba(0,0,0,0.45)]">
            <h3 className="text-2xl font-bold font-montserrat tracking-tight mb-6">{editingId ? 'Edit' : 'Add'} Resource</h3>
